...
export function QueueClient({ initialData }: QueueClientProps) {
  const { data } = useQueue(5000);
  const previousPendingCount = useRef(initialData.pending.length);

  const queueData = data || initialData;
  const currentTicket = queueData.current;
  const pendingTickets = queueData.pending || [];
  const completedToday = queueData.completedTodayCount || 0;

  // Show toast when new ticket arrives
  useEffect(() => {
    if (data && data.pending) {
      const newCount = data.pending.length;
      if (newCount > previousPendingCount.current) {
        toast.info("Tiket baru masuk!", {
          description: `Ada ${newCount - previousPendingCount.current} tiket baru di antrian`,
        });
      }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="glass rounded-2xl p-8 shadow-deep border-2 border-[#06b6d4]/20">
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <p className="text-lg text-[#64748b] mt-1">{currentTicket.user?.name || currentTicket.guestName || "Pengunjung Offline"}</p>
                </div>
              ...
              <QueueActions ticketId={currentTicket.id} status={currentTicket.status} category={currentTicket.category} />
            </div>
          ) : (
              ...
              {pendingTickets.map((ticket: TicketItem, index: number) => (
                ...
                >
                  <div className="flex items-center justify-between">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1 flex-wrap">
                          ...
                          {ticket.category === "PRIORITY" ? (
                            <span className="text-xs px-2 py-1 bg-purple-100 text-purple-700 rounded font-bold border border-purple-200">
                              LOKET 2 (PRIORITAS)
                            </span>
                          ) : (
                            <span className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded font-bold border border-blue-200">
                              LOKET 1 (UMUM)
                            </span>
                          )}
                        </div>
                        <p className="text-sm text-[#64748b]">{ticket.user?.name || ticket.guestName || "Pengunjung Offline"}</p>
                    ...
                    <QueueActions ticketId={ticket.id} status={ticket.status} category={ticket.category} />
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="space-y-6">
        ...
        <div className="glass rounded-xl p-6 shadow-card animate-slide-in-up animation-delay-200">
          <h3 className="text-display text-lg font-bold text-[#0a1628] mb-4">Statistik Hari Ini</h3>
